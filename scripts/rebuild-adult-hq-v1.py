#!/usr/bin/env python3
"""Adult Path HQ v1 — robust shared-clock lips + continuous body.
...
Usage:
  python3 scripts/rebuild-adult-hq-v1.py
  python3 scripts/rebuild-adult-hq-v1.py meal-adult
  python3 scripts/rebuild-adult-hq-v1.py --force greeting-adult
  python3 scripts/rebuild-adult-hq-v1.py --prepare-audio
"""
...
from rebuild_adult_lines import CLIPS, STUDENT_SEED  # noqa: E402
from kids_animation_lib import normalize_oral_slot  # noqa: E402

STILLS = ROOT / "film-adult" / "stills"
MOTION = ROOT / "film-adult" / "motion"
SHOTS = ROOT / "film-adult" / "shots"
EXPORT = ROOT / "film-adult" / "export"
PUBLIC = ROOT / "public" / "scenes"
AUDIO = ROOT / "public" / "audio" / "adult"
STUDENT_AUDIO = ROOT / "public" / "audio" / "student"
...
def ensure_oral(line_id: str, force: bool = False) -> Path:
    ...
    if not force and out.exists() and peak_audio(out) >= MIN_ORAL_PEAK:
        return out
    seed_id = STUDENT_SEED.get(line_id)
    if not seed_id:
        raise FileNotFoundError(f"no seed mapping for {line_id}")
    seed = STUDENT_AUDIO / f"{seed_id}.mp3"
    if not seed.exists():
        raise FileNotFoundError(seed)
    meta = normalize_oral_slot(seed, out, lead=ORAL_LEAD, total=DUR)
    peak = peak_audio(out)
...
def build_shot(clip: str, i: int, line_id: str, text: str, force: bool = False) -> Path:
    ...
    work = Path(tempfile.mkdtemp(prefix=f"av1-{clip}-{i:02d}-"))
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Adult Path HQ v1 rebuild")
    ...
    print("=== Adult Path HQ v1 ===", flush=True)
    ...
    print("\n=== ADULT HQ V1 SUMMARY ===", flush=True)
